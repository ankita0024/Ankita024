  ...
  constructor(public http: Http) {
    ...
    updateProfile(mid, genre,token):Observable<Data[]>{
      ...
      let options = new RequestOptions({ headers: headers });
          return this.http.get('https://glc-devtest-lims-api.azurewebsites.net/lims/updateUserProfile/'+mid+'/'+genre,options)
              .map((res:Response)=>res.json()) ;
            }
 getMyGenre(mid,token){
       let headers = new Headers({ 'Authorization': 'Bearer ' + token });
      let options = new RequestOptions({ headers: headers });
          return this.http.get('https://glc-devtest-lims-api.azurewebsites.net/lims/getMyFavGenre/'+mid,options)
